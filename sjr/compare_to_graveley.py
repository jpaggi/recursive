...
for line in sys.stdin:
	...
	for (g_chrom, g_start, g_end, g_strand, g_rs) in graveley:
		...
		if g_chrom == chrom and g_strand == strand:# and #(g_start <= start <= g_end or g_start <= end <= g_end):
			...
			# 	print 'match'
	if not seen:#not overlap or not seen:
		print chrom, start, end, strand, sample, entropy, seq1, seq2, rs, 'novel'
	else:
		print chrom, start, end, strand, sample, entropy, seq1, seq2, rs, 'graveley'
	# 	pass
	# else:
	# 	print seq1 + seq2
