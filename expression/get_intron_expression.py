...
for intron in introns:
    ...
    if intron_end - intron_start < MINIMUM_INTRON_SIZE: continue

    expression = [0] * (intron_end - intron_start)
    sjr_count = 0

    try:
        reads = samfile.fetch('chr'+chrom, intron_start-MAX_INSERT, intron_end+MAX_INSERT)
    except ValueError: 
        continue
    for read in reads:
        ...
        for i in range(len(blocks) - 1):
            if blocks[i][1] == intron_start and blocks[i+1][0] == intron_end:
                sjr_count += 1
